"""
   Автор: StaBur
   Дата: 29.02.2024; 0:05 GMT+3;
   Собран: 29.02.2024; 0:16 GMT+3;
...
from forma_ui import Ui_MainWindow
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        def on_click_testssh():
            ...
            ssh_port = self.lineEdit_2.text()
            ssh_login = self.lineEdit_3.text()
            ssh_pass = self.lineEdit_4.text()
            ssh_mysql_host = self.lineEdit_5.text()
            ssh_mysql_port = self.lineEdit_6.text()

            if not ssh_host:
                self.label_7.setText("<font color=red>Не заполнено поле Адрес SSH сервера!</font>")
            elif not ssh_port:
                self.label_7.setText("<font color=red>Не заполнено поле Порт SSH сервера!</font>")
            elif not ssh_login:
                self.label_7.setText("<font color=red>Не заполнено поле Логин SSH сервера!</font>")
            elif not ssh_pass:
                self.label_7.setText("<font color=red>Не заполнено поле Пароль SSH сервера!</font>")
            elif not ssh_mysql_host:
                self.label_7.setText("<font color=red>Не заполнено поле Адрес MySql сервера!</font>")
            elif not ssh_mysql_port:
                self.label_7.setText("<font color=red>Не заполнено поле Порт Mysql сервера!</font>")
            ...
            if (ssh_host and ssh_port and ssh_login and ssh_pass and ssh_mysql_host and ssh_mysql_port):
                try:
                    ...
                    if (server):
                        ...
                        self.label_7.setText("<font color=green>Соединение c SSH-сервером установленно!</font>")
                    else:
                        print("Проблема с подключением к SSH-серверу!")
                    server.start()
                except Exception as e:
                    self.label_7.setText("<font color=red>Соединение c SSH-сервером НЕ установленно!</font>")
                    print(f"""Ошибка: {e}""")
        ...
        def on_click_testbd():
            ...
            if (server):
                mysql_host = self.lineEdit_7.text()
                mysql_login = self.lineEdit_8.text()
                mysql_password = self.lineEdit_9.text()
                mysql_db_name = self.lineEdit_10.text()
                mysql_port = server.local_bind_port

                if not mysql_host:
                    self.label_7.setText("<font color=red>Не заполнено поле Хост MySql!</font>")
                elif not mysql_login:
                    self.label_7.setText("<font color=red>Не заполнено поле Логин MySql!</font>")
                elif not mysql_password:
                    self.label_7.setText("<font color=red>Не заполнено поле Пароль MySql!</font>")
                elif not mysql_db_name:
                    self.label_7.setText("<font color=red>Не заполнено поле База Данных MySql!</font>")
                ...
                if (mysql_host and mysql_login and mysql_password and mysql_db_name):
                    try:
                        ...
                        self.label_7.setText("<font color=green>Соединение c MySql установленно!</font>")
                        print(f"Соединение c MySql установленно!\n порт: {mysql_port}")
                        dbconnect.commit()
                        dbconnect.rollback()
                        dbconnect.close()
                    except:
                        self.label_7.setText("<font color=red>Соединение c MySql НЕ установленно!</font>")
            else:
                print("Переменные 'server' или 'dbconnect' не найдены!")
                self.label_7.setText("<font color=red>Нет подключения к SSH серверу!</font>")
        ...
        def on_click_excfile():
            ...
            self.label_6.setText(f"<font color=green>Файл <font color=blue>{exFile}</font> прикреплён!</font>")
        ...
        def on_click_create():
            ...
            if (server and dbconnect):
                if not mysql_host:
                    ...
                    self.label_7.setText("<font color=red>Не заполнено поле Порт MySql!</font>")
                ...
                if (mysql_host and mysql_login and mysql_password and mysql_db_name):
                    ...
                    try:
                        ...
                        self.label_7.setText(f"<font color=green>Таблица <font color=blue>{mysql_table_name}</font> в базе данных создана!!!</font>")
                        print(f"Таблица {mysql_table_name} создана")
                    except Exception as e:
                        self.label_7.setText("<font color=red>Соединение c MySql-сервером НЕ установленно!</font>")
                        print(f"""Ошибка: {e}""")
                        dbcon.rollback()
                    ...
                    for body in range(len(mysql_ef2)):
                        ...
                        try:
                            ...
                            self.label_7.setText(f"<font color=red>Содержание таблицы {mysql_table_name} не добавленно!</font></font>")
                    dbcon.commit()
                    dbcon.close()
                else:
                    self.label_7.setText("<font color=red>Ошибка заполнения!!!</font>")
            else:
                print("Переменные 'server' или 'dbconnect' не найдены!")
                self.label_7.setText("<font color=red>Нет подключения к SSH и/или MySql серверу!</font>")
        ...
        self.pushButton.clicked.connect(on_click_windows)
        self.pushButton_2.clicked.connect(on_click_testbd)
        self.pushButton_3.clicked.connect(on_click_excfile)
        self.pushButton_4.clicked.connect(on_click_create)
        self.pushButton_5.clicked.connect(on_click_testssh)
